...
def SaveTestPageHAR(output_dir, url, test_id, content):
	"""Save the page infomation of a test into a file on disk.
	...
	"""
	filename = BuildFileName(url)
	filename = '%s/%s.%s.har' % (output_dir.rstrip('/'), filename, test_id)
	output = open(filename, 'wb')
	output.write(content)
	output.close()
...
def RunBatch(options):
	...
	id_url_dict = wpt_batch_lib.SubmitBatch(requested_urls, test_params, options.server)
	...
	while pending_test_ids:
		...
		id_status_dict = wpt_batch_lib.CheckBatchStatus(pending_test_ids, server_url=options.server)
		completed_test_ids = []
		...
		test_results = wpt_batch_lib.GetXMLResult(completed_test_ids, server_url=options.server)
		...
		for test_id, dom in test_results.iteritems():
			print test_id
			SaveTestResult(options.outputdir, id_url_dict[test_id], test_id, dom.toxml())
			# Added by chen
			test_page_har = wpt_batch_lib.GetPageHAR(test_id, server_url=options.server)
			SaveTestPageHAR(options.outputdir, id_url_dict[test_id], test_id, test_page_har)
		if pending_test_ids:
